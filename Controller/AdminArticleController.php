...
class AdminArticleController extends Controller
{
     ...
     *
     * @Method("GET")
     * @Route("/index.html", name="admin_article_index")
    ...
    public function indexAction()
    {
        ...
        $keys = array(
            'id',
            'article.admin.article.name.text',
        );
     ...
     *
     * @Method("GET")
     * @Route("/list.json", name="admin_articles_json")
     * @Secure(roles="ROLE_ADMIN")
     *
     * @return Response
     */
    ...
    public function newAction()
    {
        ...
        $formHandler    = $this->get('black_article.article.form.handler');
        ...
        if ($process) {
            ...
            return $this->redirect($this->generateUrl('admin_article_edit', array('id' => $document->getId())));
        }
     ...
     *
     * @Method({"POST", "GET"})
     * @Route("/{id}/delete/{token}", name="admin_article_delete")
     * @Secure(roles="ROLE_ADMIN")
     *
    ...
    public function deleteAction($id, $token = null)
    {
        ...
        if ($form->isValid() || true === $token) {
            ...
            $this->get('session')->getFlashBag()->add('success', 'success.article.admin.article.delete');

        } else {
            $this->getFlashBag->add('error', 'error.article.admin.article.not.valid');
        }
     ...
     *
     * @Method({"POST"})
     * @Route("/batch", name="admin_article_batch")
     *
     * @return array
     *
    ...
    public function batchAction()
    {
        ...
        if (!$ids = $request->get('ids')) {
            $this->get('session')->getFlashBag()->add('error', 'error.article.admin.article.no.item');

            return $this->redirect($this->generateUrl('admin_article_index'));
        }

        if (!$action = $request->get('batchAction')) {
            $this->get('session')->getFlashBag()->add('error', 'error.article.admin.article.no.action');
        ...
        if (false === $token) {
            $this->get('session')->getFlashBag()->add('error', 'error.article.admin.article.csrf');
